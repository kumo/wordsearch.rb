...
class TestCountAvailableSpacesInColumn < Test::Unit::TestCase
  def setup
  ...
  def test_empty_column
    ...
    assert_equal @word_search.available_spaces_for_column(0), [6]
  end
  
  def test_count_no_space_left
    @word_search.add_grid([['r e d'],['r e d'],['r e d']], 3, 3)
    @word_search.generate_with_spaces(3,3)
    assert_equal @word_search.available_spaces_for_column(0), [0]
  end

  def test_spaces_at_both_ends
    @word_search.add_grid([['. . .'], ['r r r'], ['. . .']], 3, 3)
    @word_search.generate_with_spaces(3,3)
    assert_equal @word_search.available_spaces_for_column(0), [1,1]
  end

  def test_space_at_end_of_column
    @word_search.add_grid([['r e d'], ['. . .'], ['. . .']], 3, 3)
    @word_search.generate_with_spaces(3,3)
    assert_equal @word_search.available_spaces_for_column(0), [2]
  end
